...
import { Check, CheckCircle2, SpaceIcon } from "lucide-react";
          ...
          <div className="w-full sm:w-1/2 lg:w-1/3 p-2">
            <div className="p-10 border border-neutral-700 rounded-xl">
              <p className="text-4xl mb-8">
                {item.title}{" "}
                {item.title === "Pro" && (
                  <span className="bg-gradient-to-r from-orange-500 to-red-400 rounded-md text-transparent bg-clip-text text-xl mb-4 ml-2">
                    (Most Popular)
                  </span>
                )}
              </p>
              <p className="mb-8">
                <span className="text-5xl mt-6 mt-2">{item.price}</span>
